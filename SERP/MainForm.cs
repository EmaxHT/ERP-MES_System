...
namespace SERP
{
    public partial class MainForm : DevExpress.XtraBars.FluentDesignSystem.FluentDesignForm
    {
        ...
        private void flyoutPanelControl1_MouseDown(object sender, MouseEventArgs e)
        {
            PanelMouse = true;
            startPointPanel = ((FlyoutPanelControl)sender).PointToScreen(new Point(e.X, e.Y));
        }

        private void flyoutPanelControl1_MouseUp(object sender, MouseEventArgs e)
        {
            PanelMouse = false;
        }

        private void flyoutPanelControl1_MouseMove(object sender, MouseEventArgs e)
        {
            if (!PanelMouse)
                return;

            endPointPanel = ((FlyoutPanelControl)sender).PointToScreen(new Point(e.X, e.Y));
            Point position = new Point((flyoutPanel1.Location.X + (endPointPanel.X - startPointPanel.X)), (flyoutPanel1.Location.Y + (endPointPanel.Y - startPointPanel.Y)));

            startPointPanel = endPointPanel;
            (flyoutPanel1.FindForm()).Location = (flyoutPanel1.FindForm()).PointToScreen(position);
        }
